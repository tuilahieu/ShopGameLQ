...
export default function ShopAssistant({ profile }) {
  ...
  const displayedMessages = messages.map((message) => message.kind === "starter"
    ? { ...message, text: `Chào bạn, mình là ${assistantName}. Bạn muốn tìm acc Liên Quân tầm giá nào?` }
    : message);
  ...
  async function send(message) {
    ...
    try {
      ...
      setMessages((current) => [...current, {
        ...
        text: answer?.text || "Mình chưa tìm được câu trả lời. Bạn thử hỏi lại nhé.",
      ...
      setMessages((current) => [...current, {
        role: "assistant",
        animate: openRef.current,
        text: "Mình đang bận một chút. Bạn có thể xem kho acc hoặc hỏi lại sau nhé.",
        accounts: [],
  ...
  return (
    <div className={`shop-assistant${open ? " is-open" : ""}`}>
      {open && (
        <section className="shop-assistant-panel" role="dialog" aria-modal="false" aria-labelledby="shop-assistant-title">
          ...
          <div ref={messagesRef} className="shop-assistant-messages" aria-live="polite" aria-relevant="additions text">
            ...
            {(busy || restoring) && <p className="shop-assistant-typing" role="status">{restoring ? "Đang tải cuộc trò chuyện…" : `${assistantName} đang trả lời…`}</p>}
          </div>
      ...
      {!open && <button type="button" className="shop-assistant-toggle" onClick={openChat} aria-expanded={false} aria-label={`Chat với AI - ${assistantName}`}>

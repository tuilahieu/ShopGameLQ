...
- `src/modules/assistant/core/instructions.js`: hướng dẫn vai trò, giới hạn đầu ra và ngữ cảnh.
- `src/modules/assistant/core/skills.js`: các luồng hỗ trợ cố định như đơn hàng, bảo hành,
  nạp tiền và liên hệ.
- `src/modules/assistant/core/tools.js`: công cụ `search_accounts` chỉ đọc tài khoản đang bán.
- `src/modules/assistant/core/harness.js`: điều phối câu hỏi và hợp đồng provider.
